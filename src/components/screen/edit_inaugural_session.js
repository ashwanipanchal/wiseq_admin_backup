...
function Edit_Inaugural_Session() {
    ...
    const [date, setDate] = useState(moment(state?.schedule_time).format("YYYY-MM-DD"))
    const [time, setTime] = useState(moment(state?.schedule_time).format("HH:MM"))
        ...
        e.preventDefault()
          ...
          .then(result => {
              ...
              if(result.success){
                ...
                alert("Inaugural session updated successfully")
                navigate(-1)
              }
            //   setProgramList(result.programList)
    ...
    // console.log(time)
    // console.log(moment(state?.schedule_time).format("YYYY-MM-DD"))
    return (
...
export default Edit_Inaugural_Session;
