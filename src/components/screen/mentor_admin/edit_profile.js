...
import camera_img from '../../../img/video-camera.png';
...
const multi_option = ['Public Speaking', 'Emotional Intelligence', 'Leadership',]
...
function Edit_Profile_Mentor() {
    ...
    const getProfile = async() => {
        ...
        const btoken = `Bearer ${token}`;
              ...
              if(success){
                ...
                setyearsWithCompany(response.data?.yearsWithCompany)
                setjobTitle(response.data?.jobTitle)
                setfunctionalArea(response.data?.functionalArea)
                setdesignationLevel(response.data?.designationLevel)
                setyearsOfExperience(response.data?.yearsOfExperience)
                setemail(response.data?.userMeta?.email)
                setreportingManagerName(response.data?.reportingManagerName)
                setlinkedinProfileLink(response.data?.linkedinProfileLink)
                setwebsiteLink(response.data?.websiteLink)
                setSelectedBSkill(response.data?.expertise?.businessSkillsGoodAt)
                setSelectedBSkillDev(response.data?.expertise?.businessSkillsToDevelop)
                setSelectedCSkill(response.data?.expertise?.coreSkillsGoodAt)
                setSelectedCSkillDev(response.data?.expertise?.coreSkillsToDevelop)
              }
    ...
    //    useEffect(()=>{
    //     let Acarray= []
    //     for(let exper of state.expertise){
    //         let dict ={
    //             achivement:achivements
    //         }
    //         Acarray.push(dict)
    ...
    useEffect(() => {
        ...
        for (let work_his of state.workHistory || []) {
            let dict = {
                role: work_his.role,
                company_name: work_his.company_name,
                start_date: work_his.start_date,
                end_date: work_his.end_date,
            }
            Acarray.push(dict)
    ...
    const updateWorkHistory = async (e) => {
        ...
        const btoken = `Bearer ${token}`;
        // const temp = []
        // for(let history of WHinputFields){
        //     console.log(history)
        //     if(history.isMyCurrentRole){
        //         let dict = {
        //             history.jobRole,
        //             history.companyName,
        //             history.startDate,
        //             history.isMyCurrentRole
        //         }
        //         temp.push(dict)
        //     }
        // }
    ...
    const updateAchivement = async (e) => {
     ...
     // CompleteProfileTask = "overview" | "expertise" | "location" | "language" | "education" | "workHistory" | "keyAchievements" 
     const response = await res.json()
     console.log("out response", response)
     // return
     const { success } = response
                                                ...
                                                <Tabs>
                                                        ...
                                                        <form onSubmit={updateOverView}>
                                                            ...
                                                            <div className="row">
                                                                ...
                                                                <div className="col-md-6 mb-25">
                                                                    <input value={empId} onChange={e => setoverview(e.target.value)} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Employee ID" disabled />
                                                                </div>
                                                                <div className="col-md-6 mb-25">
                                                                    <input value={yearsWithCompany} maxLength={2} onChange={e => {
                                                                        const re = /^[0-9\b]+$/;
                                                                        if (e.target.value === '' || re.test(e.target.value)) {
                                                                            setyearsWithCompany(e.target.value)
                                                                        }
                                                                        
                                                                        }} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Years with the Company" required/>
                                                                </div>
                                                                <div className="col-md-6 mb-25">
                                                                    <input value={jobTitle} onChange={e => setjobTitle(e.target.value)} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Job Title" disabled />
                                                                </div>
                                                                <div className="col-md-6 mb-25">
                                                                    <input value={functionalArea} onChange={e => setfunctionalArea(e.target.value)} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Fuctional Area" disabled />
                                                                </div>
                                                                <div className="col-md-6 mb-25">
                                                                    <input value={designationLevel} onChange={e => setdesignationLevel(e.target.value)} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Level of employee" disabled />
                                                                </div>
                                                                <div className="col-md-6 mb-25">
                                                                    <input value={yearsOfExperience} maxLength={2} onChange={e => {
                                                                        const re = /^[0-9\b]+$/;
                                                                        if (e.target.value === '' || re.test(e.target.value)) {
                                                                            setyearsOfExperience(e.target.value)
                                                                        }
                                                                        }} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Years of experience" required/>
                                                                </div>
                                                                <div className="col-md-6 mb-25">
                                                                    <input value={email} onChange={e => setemail(e.target.value)} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Email address" disabled />
                                                                </div>
                                                                <div className="col-md-6 mb-25">
                                                                    <input value={reportingManagerName} onChange={e => setreportingManagerName(e.target.value)} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Reporting Manager" disabled />
                                                                </div>
                                                                <div className="col-md-6 mb-25">
                                                                    <input value={linkedinProfileLink} onChange={e => setlinkedinProfileLink(e.target.value)} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Linkedin Profile Link" />
                                                                </div>
                                                                <div className="col-md-6 mb-25">
                                                                    <input value={websiteLink} onChange={e => setwebsiteLink(e.target.value)} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Website Link" />
                                                                </div>
                                                        ...
                                                        <div className="row">
                                                            <form onSubmit={updateExpertise}>
                                                                <div className="row">
                                                                    <div className="col-md-6 mb-25">
                                                                        <div class="countryOption">
                                                                            ...
                                                                            <Multiselect
                                                                                ...
                                                                                placeholder='Business Skills I can Mentor In'
                                                                    ...
                                                                    <div className="col-md-6 mb-25">
                                                                        <div class="countryOption">
                                                                            ...
                                                                            <Multiselect
                                                                                ...
                                                                                placeholder='Core Skills I can Mentor In'
                                                                     ...
                                                                     {/* <div className="col-md-6 mb-25">
                                                                        <div class="countryOption">
                                                                            <Multiselect
                                                                                ...
                                                                                value={[{ id: 1, industry: "as" }]}
                                                                                options={coreSkills} // Options to display in the dropdown
                                                                                placeholder='Tools I am good with'
                                                                                selectedValues={selectedCSkill} // Preselected value to persist in dropdown
                                                                                onSelect={onSelectCSkill} // Function will trigger on select event
                                                                                onRemove={onRemoveCSkill} // Function will trigger on remove event
                                                                                displayValue="skill" // Property name to display in the dropdown options
                                                                    ...
                                                                    <div className="col-md-6 mb-25">
                                                                        <div class="countryOption">
                                                                            <Multiselect
                                                                                ...
                                                                                placeholder='Tools I can mentor in'
                                                                    ...
                                                                    <div className="col-md-12 mb-25">
                                                                        <textarea value={goal} onChange={(e) => setGoal(e.target.value)} class="form-control b-deep" id="exampleFormControlTextarea1" rows="3" placeholder='Aspirational Goals'></textarea>
                                                                    </div>
                                                                    {/* <div className="col-md-6 mb-25">
                                                    ...
                                                    <TabPanel className="tab-content">
                                                        <form onSubmit={updateLanguage}>
                                                            <div key={key2} className="row">
                                                                {languages.map((data, index) => (
                                                                    <>
                                                                        <div className="col-md-6 mb-25">
                                                                            <input name="language" value={data.language} onChange={(evnt) => handleChangeForLang(index, evnt)} type="text" className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Language" />
                                                                ...
                                                                <h6 className='color-petrol' onClick={() => addInputFieldLang()}>+ ADD ANOTHER LANGUAGE</h6>
                                                                <div className="layout-button">
                                                                    <div className="btn_center">

                                                                        <button type="submit" className="btn btn-petrol btn-default btn-squared">Save</button>
                                                        ...
                                                        <form onSubmit={updateEducation}>
                                                            <div key={key1} className="row">
                                                                {inputFields.map((data, index) => (
                                                                    <>
                                                                        <div className="col-md-6 mb-25">
                                                                            <input type="text" name="degree" value={data.degree} onChange={(evnt) => handleChangeForEdu(index, evnt)} className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Degree" required/>
                                                                        </div>
                                                                        <div className="col-md-6 mb-25">
                                                                            <input type="text" name="collegeName" value={data.collegeName} onChange={(evnt) => handleChangeForEdu(index, evnt)} className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="College Name" required/>
                                                                        </div>
                                                                        <div className="col-md-6 mb-25">
                                                                            <input type="number" name="passingYear" value={data.passingYear} onChange={(evnt) => handleChangeForEdu(index, evnt)} className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Passing Year" required/>
                                                                        </div>
                                                                ...
                                                                <h6 className='color-petrol' onClick={() => addInputField()}>+ ADD MORE</h6>
                                                                <div className="layout-button">
                                                                    <div className="btn_center">
                                                    ...
                                                    <TabPanel className="tab-content">
                                                        <form onSubmit={updateWorkHistory}>
                                                            <div key={key3} className="row">
                                                                {WHinputFields.map((data, index) => (
                                                                    <>
                                                                        <div className="col-md-6 mb-25">
                                                                            <input type="text" name="jobRole" value={data.jobRole} onChange={(evnt) => handleChangeForWH(index, evnt)} className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Role" required/>
                                                                        </div>
                                                                        <div className="col-md-6 mb-25">
                                                                            <input type="text" name="companyName" value={data.companyName} onChange={(evnt) => handleChangeForWH(index, evnt)} className="form-control ih-medium ip-gray b-deep radius-xs px-15" placeholder="Company Name" required/>
                                                                        </div>
                                                                        {/* <div className="col-md-6 mb-25">
                                                                            <input name="startDate" value={moment(data.startDate).format("MM/DD/YYYY")} onChange={(evnt) => handleChangeForWH(index, evnt)} onFocus={() => setType('date')}
                                                                                onBlur={() => setType('text')} type={type} className="form-control ih-medium ip-gray b-deep radius-xs px-15 date_time" placeholder="Start Date" />
                                                                        </div> */}
                                                                        <div className="col-md-6 mb-25">
                                                                            <input 
                                                                            onClick={() =>{
                                                                        ...
                                                                        {/* <div className="col-md-6">
                                                                            <input name="endDate" value={moment(data.endDate).format("MM/DD/YYYY")} onChange={(evnt) => handleChangeForWH(index, evnt)} onFocus={() => setType('date')}
                                                                                onBlur={() => setType('text')} type={type} className="form-control ih-medium ip-gray b-deep radius-xs px-15 date_time" placeholder="End Date" disabled={data.isMyCurrentRole ? true : false} />
                                                                            <div >
                                                                ...
                                                                <h6 className='color-petrol' onClick={() => addInputFieldWH()}>+ ADD MORE</h6>
                                                                <div className="layout-button">
                                                                    <div className="btn_center">
                                                    ...
                                                    <TabPanel className="tab-content">
                                                        <form onSubmit={updateAchivement}>
                                                            <div key={key} className="row">
                                                                ...
                                                                <h6 className='color-petrol' onClick={() => addInputFieldAc()}>+ ADD MORE</h6>
