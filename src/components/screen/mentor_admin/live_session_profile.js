// import resources_img from '../../../img/live_session.png';
...
function Live_Session() {
  ...
  const rtcProps = {
    ...
    channel: state.id, // your agora channel
...
// {recordStatus == true ? null : startCall()}
  const startCall = async() => {
    ...
    const res = await fetch(`${BASE_URL}session-video-conference/${state.id}/start`, {
        method: 'PUT',
        headers: {
          "Accept": "application/json",
          'Content-Type': 'application/json',
                  ...
                  <div className=" d-flex flex-wrap justify-content-center breadcrumb-main__wrapper">
                    <div className="d-flex align-items-center user-member__title justify-content-center me-sm-25">
                      <h4 className="text-capitalize fw-500 breadcrumb-title">
                        ...
                        {new Date(fullSessionDetail.scheduleTime).toDateString()} with{" "}
                        {fullSessionDetail.scheduler?.name}{" "}
                        {new Date(fullSessionDetail.scheduleTime).toLocaleTimeString()}
                      </h4>
                    </div>
                  </div>
                </div>
