...
function Live_Session() {
  ...
  const getSessionProfile = async() => {
    ...
    if(response.success){
      ...
      let mt = []
      let mr = []
      response.session[0].session_users?.map((i) => {
        if(i.role == "mentee"){
          mt.push(i)
        }else{
          mr.push(i)
      ...
      setData([
        { id: 1, mentoring_hours: "Mentees", mentoring_num: mt.length },
        { id: 2, mentoring_hours: "Mentors", mentoring_num: mr.length },
    ])
      // setMentors(mr)
      // setMentees(mt)
    }
...
const saveNotes = async () => {
  ...
  const body = {
    "session_id": fullSessionDetail?.id,
    "org_notes":notes
  ...
  console.log(body)
  // return
  const res = await fetch(`${BASE_URL_APPLSURE_MENTORING}program-session-add-notes`, {
      method: 'POST',
      headers: {
          "Accept": "application/json",
          'Content-Type': 'application/json',
            ...
            <div className="row">
              ...
              <div className="col-lg-6 col-md-12 col-sm-12">
                <h5 className="text-capitalize fw-500 mb-15">
                  Session Number #{fullSessionDetail.session_number}
                </h5>
                <h5 className="text-capitalize fw-500 mb-15">
                 Title
                </h5>

                {state?.session?.type != "iaugural" && (
                  <>
              <h5 className="text-capitalize fw-500 mb-15">
                  Session Overview
                </h5>
                ...
                <div className="col-md-12 mb-20">
                    ...
                    <ul className="d-flex flex-wrap user-group-people__parent">
                            <span key={1} class="badge badge-square btn-outline-orange me-10">{fullSessionDetail && fullSessionDetail?.skills}</span>
                    </ul>
                </div>
                  </>
                )}
                ...
                <div className="col-lg-12 col-md-12">
                  <button
                    ...
                    type="button"
                    class="btn btn-primary btn-squared color-primary px-15 mb-20"
                  >
                    Save
                  </button>
