...
function Preferences_Question() {
    ...
    const [question, setQuestion] = useState("What brings you to the platform (you may select one or multiple)")
    ...
    const getQues = async () => {
        ...
        const res = await fetch(`http://13.235.104.81:8000/api/mentee/preferences`, {
            method: 'GET',
            headers: {
                                                    ...
                                                    <div class="prefernce_ques mb-25">
                                                        <div key={key} className="prefer_img d-flex">
                                                            <div onClick={() => updateCheck(index)} style={{ backgroundColor: i.priority != "" ? "#72b8bf" : "lightgray", height: 30, width: 30, borderRadius: 15 }}>
                                                                <p style={{ textAlign: 'center', color: 'white', marginTop: "3px" }}></p>
                                                            </div>
